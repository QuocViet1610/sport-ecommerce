...
function displayAddresses(addresses) {
                ...
                <div class="delivery-address-box">
                    <div>
                        ...
                        <ul class="delivery-address-detail">
                            <li><h4 class="fw-500">${address.fullName}</h4></li>
                            <li><p class="text-content" data-ward-id="${address.wardId}" data-district-id="${address.districtId}" data-province-id="${address.provinceId}">
                                    <span class="text-title">Địa chỉ: </span>${address.addressText}, ${address.wardName}, ${address.districtName}, ${address.provinceName}
                                </p>
                            </li>
                            <li><h6 class="text-content mb-0"><span class="text-title">Số điện thoại: </span>${address.phoneNumber}</h6></li>
                            <li>
    ...
    // Reset form khi mở modal để thêm địa chỉ mới
    $(document).on('click', '#openModalBtnAddress', function() {
        ...
        $("#addressForm").data('addressId', null); // Đặt lại data-id của form khi thêm mới
    });

    // Xử lý khi click vào nút Lưu (thêm mới hoặc cập nhật)
    $("#submitAddress").on("click", function(event) {
        ...
        let addressId = $("#addressForm").data('addressId');  // Lấy addressId từ form

        if (addressId) {
            // Cập nhật địa chỉ
            $.ajax({
                url: `http://localhost:8080/addresses/update/${addressId}`,  // URL cập nhật
                method: "PUT",
                ...
                error: function(xhr, status, error) {
                    let errorMessage = "Đã có lỗi xảy ra khi cập nhật địa chỉ.";
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        errorMessage = xhr.responseJSON.message;
                    } else if (xhr.responseText) {
                        errorMessage = xhr.responseText;
            ...
            // Thêm mới địa chỉ
            $.ajax({
                url: "http://localhost:8080/addresses",  // URL thêm mới
                method: "POST",
                contentType: "application/json",
                data: JSON.stringify(formData),
                headers: {
