...
function renderProductAttributes(products) {
    ...
    $('.attribute-value').on('click', function() {
        ...
        if (selectedVariant.length > 0) {
            $('#nameProduct').text(selectedVariant.name); 
            // $('#Price').text(selectedVariant.price.toLocaleString() + "đ"); 
            // $('#PriceOld').text(selectedVariant.price.toLocaleString() + "đ"); 
            // $('#brandName').text(product.brandName || 'Không có thương hiệu');  
            $('#productCode').text(selectedVariant.code || 'Không có thông tin'); 
            $('#createdAt').text(new Date(selectedVariant.createdAt).toLocaleDateString() || 'Không có thông tin');   
            console.log(selectedVariant);
        } else {
