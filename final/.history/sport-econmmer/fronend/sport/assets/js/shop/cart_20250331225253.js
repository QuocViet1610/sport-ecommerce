...
function updateProductSubtotal($input, quantity) {
    ...
    // Cập nhật tổng cộng trong giỏ hàng, giữ 2 chữ số thập phân
    totalPrice = totalPrice.toFixed(2); // Đảm bảo làm tròn đến 2 chữ số thập phân

    // Cập nhật tổng cộng trong giỏ hàng
    $row.find('.totalProduct').text(totalPrice.toLocaleString('vi-VN') + 'đ'); // Sử dụng 'vi-VN' cho định dạng tiền tệ
