...
function updateCartQuantity(productId, productVariantId, quantity, totalPrice) {
    ...
    function updateCartQuantity(productId, productVariantId, quantity, totalPrice) {
        ...
        let product = cart.find(item => {
            // Kiểm tra nếu `productVariants` tồn tại, là mảng và có nhiều hơn 1 phần tử
            if (item.productVariants && Array.isArray(item.productVariants) && item.productVariants.length > 1) {
                return item.productId === productId && 
                       item.productVariants[0].id === productVariantId;
            } else {
                // Nếu không có `productVariants` hoặc số lượng `productVariants` <= 1, kiểm tra `productId` và `productVariantId`
