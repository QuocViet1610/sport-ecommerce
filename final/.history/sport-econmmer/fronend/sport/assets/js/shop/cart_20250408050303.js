...
function renderCart() {
    ...
    <tr class="product-box-contain">
        <td colspan="5" class="text-center"><strong>Giỏ hàng trống</strong></td>
    </tr>
            `;
            $tbody.html(cartHTML);
    ...
    if (cart.length > 0) {
        ...
        $(document).on('click', '.delete-product', function () {
            if(token){
                ...
                $.ajax({
                    url: `http://localhost:8080/cart/remove/${cartItemId}`,  // Đường dẫn đến API của bạn
                    method: 'DELETE',
                    headers: {
                        'Authorization': 'Bearer ' + token,
                        'Content-Type': 'application/json',
...
function updateProvisionalPrice() {
    ...
    if (totalProductElements.length > 0) {
        ...
        document.getElementById('provisionalPrice').innerText = totalPrice) + "đ"; 
