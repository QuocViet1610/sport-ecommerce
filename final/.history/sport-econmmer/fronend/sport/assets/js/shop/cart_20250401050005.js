...
function updateProvisionalPrice() {
    let totalPrice = 0;
    // Lấy tất cả các phần tử có class "totalProduct" (các giá trị tổng cho mỗi sản phẩm)
    let totalProductElements = document.querySelectorAll('.totalProduct');

    console.log(totalProductElements);  // Debugging xem có phần tử nào được chọn không

    if (totalProductElements.length > 0) {
        // Dùng vòng lặp for để cộng tất cả giá trị trong các phần tử này
        for (let i = 0; i < totalProductElements.length; i++) {
            let priceText = totalProductElements[i].innerText.replace('đ', '').replace(',', ''); // Lấy giá trị, loại bỏ ký tự 'đ' và dấu ',' nếu có
            let price = parseFloat(priceText); // Chuyển đổi thành số
            console.log(price);
            if (!isNaN(price)) {  // Kiểm tra nếu là giá trị hợp lệ
                totalPrice += price; // Cộng vào tổng
            }
        }

        // Cập nhật tổng vào phần tử với id "provisionalPrice"
        document.getElementById('provisionalPrice').innerText = totalPrice.toLocaleString() + "đ"; // Định dạng số và thêm đơn vị "đ"
    } else {
