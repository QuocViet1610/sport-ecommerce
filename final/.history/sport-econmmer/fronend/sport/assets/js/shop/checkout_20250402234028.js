...
$(document).ready(function() {
    ...
    async function setDistrictAndWard(provinceId, districtId, wardId, districtName, wardName) {
        try {
            await renderPlaceUpdate();  // Đảm bảo renderPlaceUpdate hoàn tất trước khi tiếp tục
    
            // Cập nhật giá trị city và trigger sự kiện change để tải districts
            $("#city").val(provinceId).trigger('change');  
    
            // Chờ cho district được load và trigger change event
            await new Promise(resolve => {
                $('#city').trigger('change');  // Trigger lại event change cho #city để tải district
                const interval = setInterval(() => {
                    const districtOptions = $('#district option').length;
                    if (districtOptions > 1) {  // Kiểm tra xem danh sách quận huyện đã được tải chưa
                        clearInterval(interval);
                        resolve();  // Nếu danh sách đã tải, tiếp tục
                    }
                }, 200);  // Kiểm tra cứ mỗi 200ms
            });
    
            // Cập nhật giá trị district và trigger sự kiện change để tải wards
            $("#district").val(districtId).trigger('change');  
    
            // Chờ cho wards được load và trigger change event
            await new Promise(resolve => {
                $('#district').trigger('change');  // Trigger lại event change cho #district để tải ward
                const interval = setInterval(() => {
                    const wardOptions = $('#ward option').length;
                    if (wardOptions > 1) {  // Kiểm tra xem danh sách phường xã đã được tải chưa
                        clearInterval(interval);
                        resolve();  // Nếu danh sách đã tải, tiếp tục
                    }
                }, 200);  // Kiểm tra cứ mỗi 200ms
