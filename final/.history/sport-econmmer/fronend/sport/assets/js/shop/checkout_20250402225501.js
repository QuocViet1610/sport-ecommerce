...
$(document).ready(function() {
    ...
    function setDistrictAndWard(provinceId, districtId, wardId, districtName, wardName) {
        $.ajax({
            ...
            success: function(response) {
                ...
                setWard(districtId, wardId, wardName); // Gọi setWard sau khi district được cập nhật
            },
            error: function(xhr, status, error) {
    ...
    function setWard(districtId, wardId, wardName) {
        $.ajax({
            ...
            success: function(response) {
                if (response.code === "200") {
                    var wardHtml = `<option value="${wardId}">${wardName}</option>`;  // Giữ phường/xã mặc định
                    response.data.forEach(function(ward) {
                        wardHtml += `<option value="${ward.WardCode}" ${ward.WardCode === wardId ? 'selected' : ''}>${ward.WardName}</option>`;
                    });
