...
function renderProductAttributes(products) {
    ...
    $('.attribute-value').on('click', function() {
        ...
        // Lọc các variant dựa trên các giá trị đã chọn và cập nhật thông tin
    ...
    // Lọc các variant dựa trên các giá trị mặc định đã chọn và cập nhật thông tin
    updateSelectedVariant();
}

function updateSelectedVariant() {
    // Lọc các variant dựa trên các giá trị trong selectedAttributeIds
    var selectedVariant = productVariantsArray.filter(function(variant) {
        var variantAttributes = variant.variantAttributes.map(function(attr) {
            return { attributeValueId: attr.attributeValueId };  // Lấy attributeValueId từ variantAttributes
        });
        ...
        return selectedAttributeIds.every(function(selected) {
            return variantAttributes.some(function(variantAttribute) {
                return variantAttribute.attributeValueId === selected.valueId;
            });
        });
    });
    ...
    // Hiển thị thông tin variant nếu tìm thấy
