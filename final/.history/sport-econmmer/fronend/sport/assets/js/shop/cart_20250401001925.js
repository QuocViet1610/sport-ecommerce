...
function updateProductSubtotal($input, quantity) {
    ...
    let productPrice = parseInt($row.find('.price h5').text().replace('đ', '').replace('.', '').trim());
    let productPriceTest = $row.find('.price h5').text().replace('đ', '').replace(/\./g, ''); 

    console.log(productPriceTest)
