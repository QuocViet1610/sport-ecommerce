...
$(document).ready(function() {
    ...
    function setDistrictAndWard(provinceId, districtId, wardId, districtName, wardName) {
        renderPlaceUpdate().then(function() {
            ...
            // Tạo một deferred object để đợi sự kiện change trên #city
            var cityChangeDeferred = $.Deferred();
            
            // Lắng nghe sự kiện 'change' trên #city
            $("#city").on('change', function() {
                // Khi sự kiện 'change' xảy ra, resolve deferred object
                cityChangeDeferred.resolve();
            });
            
            // Đợi cho đến khi sự kiện 'change' trên #city hoàn tất
            cityChangeDeferred.done(function() {
                // Sử dụng .done để thực hiện các thao tác tiếp theo khi 'change' hoàn tất
                var districtHtml = `<option value="${districtId}" selected>${districtName}</option>`;
                $("#district").append(districtHtml);
                // Cập nhật giá trị cho #district và trigger lại sự kiện change
