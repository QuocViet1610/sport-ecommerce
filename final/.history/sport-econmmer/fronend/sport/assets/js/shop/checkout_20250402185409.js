...
var deleteAddressId = null;  // Biến lưu trữ ID địa chỉ cần xóa

// Lắng nghe sự kiện click vào nút "Xóa"
$(document).on('click', '.delete-btn', function() {
    // Lấy id của địa chỉ cần xóa
    deleteAddressId = $(this).closest('div[id]').attr('id');
...
// Khi người dùng nhấn "Đồng ý" xóa
confirmDeleteBtn.click(function() {
    $.ajax({
        url: `http://localhost:8080/addresses/remove/${deleteAddressId}`,  // URL API xóa địa chỉ
        type: 'DELETE',
        headers: {
            'Authorization': 'Bearer ' + localStorage.getItem('authToken')  // Thêm token vào header nếu cần
        },
        success: function(response) {
            $("#" + deleteAddressId).remove();  // Xóa địa chỉ khỏi DOM
        ...
        error: function(xhr, status, error) {
            ...
            modal.hide();  // Đóng modal nếu có lỗi
        }
    });
});

// Khi người dùng nhấn "Hủy", đóng modal
