...
function updateCartQuantity(productId, productVariantId, quantity, totalPrice) {
    ...
    if (product) {
        // Cập nhật số lượng và giá trị tổng tiền nếu sản phẩm có variant
        if (product.productVariants && product.productVariants.length > 0 && productVariantId) {
            let variant = product.productVariants.find(variant => variant.id === productVariantId);
            if (variant) {
                variant.quantityCart = quantity; // Cập nhật số lượng của variant
            }
        } else {
