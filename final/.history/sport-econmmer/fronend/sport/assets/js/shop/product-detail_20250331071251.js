...
function addToCart() {
    ...
    if(selectedAttributeIds && selectedAttributeIds.length > 0){
        ...
        if (selectedVariant.length > 0) {
            console.log("Product variant selected:", productSave);

            // Kiểm tra xem giỏ hàng đã tồn tại trong localStorage chưa
            let cart = JSON.parse(localStorage.getItem('Cart'));
            if (!cart) {
                cart = [];  // Nếu chưa có, khởi tạo giỏ hàng mới là mảng rỗng
            }
            ...
            console.log("Product variant selected:", productSave);
            localStorage.setItem('Cart', JSON.stringify(productSave));
            console.log("Variant added to cart:", JSON.parse(localStorage.getItem('Cart')))
            console.log("No matching product variant found.");
    }
