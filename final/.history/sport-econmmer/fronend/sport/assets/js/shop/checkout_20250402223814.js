...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.code === "200") {
                    ...
                    // Cập nhật lại dropdown tỉnh/thành phố
                    $("#city").val(address.provinceId).trigger('change');  // Cập nhật lại và gọi sự kiện change để load các quận
                    
                    // Cập nhật lại quận/huyện và phường/xã
                    setDistrictAndWard(address.provinceId, address.districtId, address.wardId, address.wardName, address.districtName);
    
                    $("#acceptTerms").prop("checked", address.isDefault === 1);
                    $("#addressModal").show();  // Mở modal
    ...
    function setDistrictAndWard(provinceId, districtId, wardId) {
        // Lấy danh sách quận/huyện từ API hoặc từ dữ liệu có sẵn
        $.ajax({
            url: `http://localhost:8080/districts/${provinceId}`,  // URL để lấy danh sách quận/huyện cho tỉnh
            type: "GET",
            headers: {
                'Authorization': 'Bearer ' + token
            },
            success: function(response) {
                if (response.code === "200") {
                    ...
                    var districtHtml = '<option value="">Chọn Quận/Huyện</option>';
                    response.data.forEach(function(district) {
                        districtHtml += `<option value="${district.id}" ${district.id === districtId ? 'selected' : ''}>${district.name}</option>`;
                    });
                    $("#district").html(districtHtml); // Cập nhật lại dropdown quận/huyện
    
                    // Sau khi quận/huyện đã được cập nhật, lấy danh sách phường/xã
                    setWard(districtId, wardId);
                }
            },
            error: function(xhr, status, error) {
    ...
    function setWard(districtId, wardId) {
        // Lấy danh sách phường/xã từ API hoặc từ dữ liệu có sẵn
        $.ajax({
            url: `http://localhost:8080/wards/${districtId}`,  // URL để lấy danh sách phường/xã cho quận
            type: "GET",
            headers: {
                'Authorization': 'Bearer ' + token
            },
            success: function(response) {
                if (response.code === "200") {
                    ...
                    var wardHtml = '<option value="${ward.id}">Chọn Phường/Xã</option>';
                    response.data.forEach(function(ward) {
                        wardHtml += `<option value="${ward.id}" ${ward.id === wardId ? 'selected' : ''}>${ward.name}</option>`;
                    });
                    $("#ward").html(wardHtml); // Cập nhật lại dropdown phường/xã
                }
            },
            error: function(xhr, status, error) {
                let errorMessage = "Lỗi khi tải phường/xã";
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    errorMessage = xhr.responseJSON.message; 
                } else if (xhr.responseText) {
                    errorMessage = xhr.responseText; 
