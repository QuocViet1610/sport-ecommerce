...
$.ajax({
    ...
    success: function(response) {
        // Kiểm tra nếu có dữ liệu và render sản phẩm
        if ( response.data.content) {
            renderProducts(response.data.content);
        } else {
            console.error("Dữ liệu sản phẩm không có hoặc không hợp lệ");
        }
    },
    error: function(xhr, status, error) {
        console.error("Đã có lỗi xảy ra: " + error);
        console.error("Chi tiết lỗi: " + xhr.responseText);  // In ra thông tin chi tiết về lỗi
    }
});
});
