...
function updateCartQuantitybackend() {
    let token = localStorage.getItem('authToken');
    
    let cartItemId = $(this).closest('tr').data('Item-id');
    let quantity = parseInt($(this).closest('.input-group').find('.qty-input').val());
    
    // Gửi PUT request để cập nhật số lượng giỏ hàng
    $.ajax({
        url: `http://localhost:8080/cart/update/${cartItemId}`,  // API URL của backend
        method: 'PUT',
        headers: {
            'Authorization': 'Bearer ' + token,
            'Content-Type': 'application/json',
