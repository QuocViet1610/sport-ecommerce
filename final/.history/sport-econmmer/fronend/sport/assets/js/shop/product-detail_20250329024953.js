    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.code === "200") {
                ...
                $('#categoryName').text(product.categoryName || 'Không có thông tin');  // Cập nhật thể loại
                $('#productCode').text(product.productCode || 'Không có thông tin');  // Cập nhật mã sản phẩm
                $('#createdAt').text(new Date(product.createdAt).toLocaleDateString() || 'Không có thông tin');  // Cập nhật ngày bán
                $('#stock').text(product.productStock || 'Không có thông tin');  // Cập nhật số lượng
