...
function addToCart() {
    ...
    if (selectedAttributeIds && selectedAttributeIds.length > 0) {
        ...
        productSave.productVariants = selectedVariant;

        if (selectedVariant.length > 0) {
            // Kiểm tra nếu sản phẩm variant này đã có trong giỏ hàng*******************************
            let existingProductIndex = cart.findIndex(function (item) {
                return item.productId === productSave.productId && item.productVariants[0].id === selectedVariant[0].id;
            });
            ...
            if (existingProductIndex !== -1) {
                ...
                // Thêm mới variant vào giỏ hàng
                productSave.quantity = quantity;
                cart.push(productSave);
            }

            localStorage.setItem('Cart', JSON.stringify(cart));
            console.log("Variant added to cart:", cart);
        } else {
            console.log("No matching product variant found.");
        }
    } else {
        // Kiểm tra nếu variant rỗng (không có variant)
        let existingProductIndex = cart.findIndex(function (item) {
            return item.productId === productSave.productId && item.productVariants.length === 0;
        });

        if (existingProductIndex !== -1) {
            // Cập nhật số lượng của sản phẩm đã có trong giỏ hàng của product
            cart[existingProductIndex].quantityCart += quantity;
        } else {
            // Thêm mới sản phẩm vào giỏ hàng nếu chưa có
            productSave.quantityCart = quantity;
            cart.push(productSave);
        }
