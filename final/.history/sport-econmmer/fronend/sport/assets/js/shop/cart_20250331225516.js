...
function updateProductSubtotal($input, quantity) {
    ...
    totalPrice = totalPrice.toFixed(2);  // Đảm bảo làm tròn đến 2 chữ số thập phân
    $row.find('.totalProduct').text(totalPrice.toLocaleString('vi-VN') + 'đ');  // Định dạng tiền theo Việt Nam
...
function updateCartQuantity(productId, quantity, totalPrice) {
    ...
    if (product) {
        ...
        product.totalPrice = totalPrice; // Cập nhật lại tổng giá trị sản phẩm
    }

    // Tính lại tổng tiền giỏ hàng
    let totalAmount = 0;
    cart.forEach(item => {
        totalAmount += parseFloat(item.totalPrice) * item.quantity;  // Cộng tổng tiền (giá * số lượng)
    });

    // Đảm bảo rằng tổng tiền giỏ hàng luôn có 2 chữ số thập phân
    totalAmount = totalAmount.toFixed(2);  // Đảm bảo làm tròn đến 2 chữ số thập phân
...
function displayTotalAmount(totalAmount) {
    $('#totalProduct').text(totalAmount.toLocaleString('vi-VN') + "đ");  // Định dạng tiền theo Việt Nam
}
