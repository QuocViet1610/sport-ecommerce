...
function updateProductSubtotal($input, quantity) {
    ...
    let productVariantId = $input.data('variant-id'); // Thêm biến để lấy `variant-id` nếu có

    // Kiểm tra sản phẩm có variant hay không
    let totalPrice = productPrice * quantity;
    if (productVariantId) {
        // Nếu có variant, lấy giá của variant
        let variantPrice = getVariantPrice(productId, productVariantId);  // Hàm giả sử lấy giá của variant
...
function updateCartQuantity(productId, productVariantId, quantity, totalPrice) {
    ...
    // Tìm sản phẩm có `productId` và `productVariantId`
    ...
    if (productIndex !== -1) {
        // Cập nhật số lượng và tổng cộng của sản phẩm variant
        cart[productIndex].quantityCart = quantity;
        cart[productIndex].totalPrice = totalPrice;
    }
                        ...
                        <div class="quantity-price">
                            <div class="cart_qty">
                                <div class="input-group">
                                    ...
                                    <input class="form-control input-number qty-input" type="text" name="quantity" value="${item.quantityCart}" data-product-id="${item.productId}" readonly>
