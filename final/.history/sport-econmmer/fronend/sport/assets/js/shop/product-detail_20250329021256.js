...
function renderProductAttributes(attributes) {
    attributes.forEach(function(attribute) {
        ...
        `;
        // Thêm thuộc tính vào phần tử phù hợp (size hoặc color)
        if (attribute.attributeName.toLowerCase().includes("size")) {
            $('#size-list').html(attributeHtml); // Render vào Kích thước
        } else if (attribute.attributeName.toLowerCase().includes("color")) {
            $('#color-list').html(attributeHtml); // Render vào Màu sắc
        }
    });
}