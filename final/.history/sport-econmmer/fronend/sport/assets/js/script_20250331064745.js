...
$(document).ready(function () {
 ...
 window.showSuccess = function (message) {
    ...
    $(".success-message").remove(); // Xóa thông báo cũ (nếu có)
    ...
    $("body").append(successHtml); // Thêm vào cuối trang

    // Đảm bảo rằng fade-out không gây ra vấn đề khi bấm lại
    $(".success-message").removeClass("fade-out"); // Xóa fade-out nếu có
    setTimeout(function () {
        $(".success-message").addClass("fade-out");
        setTimeout(function () {
            $(".success-message").remove();
        }, 500); // Sau khi fade-out xong, xóa hoàn toàn
    }, 2000); // Đảm bảo thời gian đủ để thấy thông báo trước khi ẩn
};
