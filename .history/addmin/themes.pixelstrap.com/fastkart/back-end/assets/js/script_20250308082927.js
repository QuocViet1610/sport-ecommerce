...
$(document).ready(function () {
    // Hàm hiển thị thông báo lỗi từ bất kỳ API nào
    window.showError = function (message) {
        ...
        }, 4000); // 🟢 Hiển thị 4 giây trước khi ẩn
    };

    // Tự động bắt lỗi từ tất cả các request AJAX
    $(document).ajaxError(function (event, xhr) {
        if (xhr.responseJSON && xhr.responseJSON.message) {
            showError(xhr.responseJSON.message); // Hiển thị thông báo từ API
        } else {
            showError("Lỗi hệ thống! Vui lòng thử lại."); // Lỗi mặc định
        }
    });
});
