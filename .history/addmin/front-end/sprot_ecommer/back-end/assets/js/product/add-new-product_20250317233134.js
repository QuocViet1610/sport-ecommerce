...
addAttributeBtn.addEventListener("click", () => {
    ...
    deleteBtn.onclick = () => {
        ...
        const valueIdsToRemove = Array.from(valueContainer.querySelectorAll(".attribute-option"))
        .map(option => option.dataset.valueId)
        .filter(id => id); // Lọc ra những ID hợp lệ

    // Xóa các giá trị khỏi danh sách đã chọn
