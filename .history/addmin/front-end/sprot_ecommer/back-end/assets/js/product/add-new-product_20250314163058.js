    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                ...
                // Log các mục cha và con để kiểm tra
                console.log('Parents:', parents); // Log cha
                console.log('Children:', children); // Log con
                ...
                // Thêm các mục parent vào select
                parents.forEach(function(parent) {
                    // Thêm mục cha vào select
                    var option = $('<option>', {
                        value: parent.id,
                        text: parent.name
                    });
                    ...
                    if (children[parent.id]) {
                        console.log('Adding children for parent ID:', parent.id); // Console log khi thêm mục con
    
                        // Duyệt các mục con của cha hiện tại
                        children[parent.id].forEach(function(child) {
                            console.log('Child:', child); // Console log mỗi mục con
                            ...
                            // Kiểm tra xem có con của mục con hay không (nếu có, tiếp tục lặp)
                            if (children[child.id]) {
                                console.log('Adding children for grandchild ID:', child.id); // Console log khi thêm mục con của mục con
                                
                                children[child.id].forEach(function(grandChild) {
                                    console.log('Grandchild:', grandChild); // Console log mỗi mục con của mục con
                                    var grandChildOption = $('<option>', {
                                        value: grandChild.id,
                                        text: '— ' + grandChild.name, // Thêm dấu gạch ngang để thụt vào thêm 1 lần nữa
                                        style: 'padding-left: 40px !important;' // Thụt vào thêm
                                    });
    
                                    // Thêm mục con của mục con vào select
