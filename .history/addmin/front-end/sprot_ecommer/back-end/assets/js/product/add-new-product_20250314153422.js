...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                // Tạo danh sách thể loại, thụt lề theo parentId
                appendCategories(response.data, categorySelect, null, 0);
            } else {
                categorySelect.append('<option value="" disabled>Không có dữ liệu thể loại</option>');
            }
        },
        error: function(xhr, status, error) {
...
// Hàm để thêm thể loại vào dropdown
function appendCategories(categories, parentSelect, parentId, level) {
    categories.forEach(item => {
        if (item.parentId === parentId) {
            let indent = "&nbsp;&nbsp;".repeat(level);  // Thêm khoảng trắng để thụt lề
            parentSelect.append(`<option value="${item.fullParentId}">${indent}${item.name}</option>`);

            // Gọi lại hàm đệ quy để thêm các thể loại con
            appendCategories(categories, parentSelect, item.parentId, level + 1);
        }
    });
}
