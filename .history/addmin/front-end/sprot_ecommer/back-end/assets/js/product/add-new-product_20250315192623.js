...
document.querySelectorAll(".price").forEach(function(input) {
    input.addEventListener("input", function() {
        let value = this.value.replace(/[^0-9]/g, ''); // Xóa tất cả các ký tự không phải số
        if (value) {
            this.value = Number(value).toLocaleString(); // Định dạng số với dấu phẩy
        } else {
            this.value = ''; // Nếu không có giá trị thì để trống
        }
    });
});
