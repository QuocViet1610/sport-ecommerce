...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                // 🔹 Sắp xếp danh mục theo fullParentId (nếu không có thì đưa lên đầu)
                response.data.sort((a, b) => {
                    let fullA = a.fullParentId || "";
                    let fullB = b.fullParentId || "";
                    return fullA.localeCompare(fullB, undefined, { numeric: true });
                });
    
                console.log("Sorted Categories:", response.data); // 🛠 Log danh mục đã sắp xếp
    
                // 🔹 Nhóm danh mục theo parentId
                let categoryMap = {};
                response.data.forEach(category => {
                    let parentKey = category.parentId !== null ? String(category.parentId) : "root";
                    if (!categoryMap[parentKey]) {
                        categoryMap[parentKey] = [];
                    }
                    categoryMap[parentKey].push(category);
                });
    
                // 🔹 Gọi hàm để render danh mục theo đúng cấp
                renderCategoryOptions(categoryMap, "root", 0, categorySelect);
            } else {
                console.warn("Không có dữ liệu danh mục.");
                categorySelect.append('<option value="" disabled>Thể loại chưa có</option>');
            }
        },
        error: function(xhr, status, error) {
            console.error("Lỗi khi lấy danh mục:", xhr.responseText || error);
            let categorySelect = $("#category");
            categorySelect.empty();
            categorySelect.append('<option value="" disabled>Không thể tải danh mục</option>');
        }
    });
    
    
    
    function renderCategoryOptions(categoryMap, parentId, level, categorySelect) {
        if (!categoryMap[parentId]) return;
    
        categoryMap[parentId].forEach(category => {
            let indent = "&nbsp;".repeat(level * 4); // Thụt vào 4 khoảng trắng mỗi cấp
            categorySelect.append(`<option value="${category.name}">${indent}${category.name}</option>`);
    
            // Gọi tiếp danh mục con với `category.id`
            renderCategoryOptions(categoryMap, String(category.id), level + 1, categorySelect);
        });
    }
});
