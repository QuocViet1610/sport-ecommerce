...
$(".attribute-group").each(function () {
    // Lấy giá trị của thuộc tính từ thẻ select trong mỗi attribute-group
    let attributeId = parseInt($(this).find(".attribute-dropdown").val());

    // Tạo mảng để lưu các giá trị thuộc tính
    let attributeValues = [];

    // Lấy tất cả các thẻ select bên trong attribute-group hiện tại
    $(this).find(".attribute-dropdown").each(function () {
        let value = $(this).val();  // Lấy giá trị của mỗi select
        console.log("test" + value);
        if (value) {
            attributeValues.push(value);  // Thêm giá trị vào mảng attributeValues nếu có
        }
    });

    // In ra giá trị thuộc tính đã chọn
    console.log(attributeValues);

    // Nếu có giá trị thuộc tính và các giá trị thuộc tính không rỗng, thêm vào productData
