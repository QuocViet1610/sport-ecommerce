            ...
            if (clickedRow) {
                ...
                if (isVisible) {
                    ...
                    $(".select-product").on('change', function () {
                        ...
                        $(`.select-variant[data-parent="${productId}"]`).prop('checked', isChecked);
                        setTimeout(() => {
                            isCheckboxChanging = false;  // Đặt lại flag sau khi đã thay đổi trạng thái checkbox
                            console.log("isCheckboxChanging (after):", isCheckboxChanging);  // Kiểm tra giá trị flag sau khi thay đổi
                        }, 0);
                    });
