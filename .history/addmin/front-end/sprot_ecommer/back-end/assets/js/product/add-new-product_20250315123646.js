...
addAttributeBtn.addEventListener("click", () => {
    ...
    $.ajax({
        url: "http://localhost:8080/brand",  // URL API lấy danh sách thương hiệu
        type: "GET",
        ...
        success: function(response) {
            if (response.code == 200) {
                const brands = response.data; // Dữ liệu thương hiệu từ API
    
                // Lấy phần tử ul của dropdown
                const dropdownMenu = $('#myDropdownBrand');
                dropdownMenu.empty();  // Xóa tất cả các mục cũ trong dropdown
    
                // Thêm ô tìm kiếm vào dropdown
                dropdownMenu.append('<li><input type="text" class="form-control" id="myInput" placeholder="Search..." onkeyup="filterFunction()"></li>');
    
                // Duyệt qua các thương hiệu và thêm từng mục vào dropdown
                brands.forEach(brand => {
                    const listItem = `<li><a class="dropdown-item" href="#" data-id="${brand.id}">${brand.name}</a></li>`;
                    dropdownMenu.append(listItem);
                });
    
                // Lắng nghe sự kiện click để hiển thị thương hiệu đã chọn
                $('.dropdown-item').on('click', function() {
                    const selectedBrandName = $(this).text();  // Lấy tên thương hiệu đã chọn
                    const selectedBrandId = $(this).data('id');  // Lấy ID của thương hiệu
    
                    // Cập nhật nút dropdown với tên thương hiệu
                    $('#dropdownButtonBrand').text(selectedBrandName);
    
                    // Lưu ID vào một biến hoặc xử lý dữ liệu
                    console.log('ID của thương hiệu đã chọn:', selectedBrandId);
    
                    // Bạn có thể hiển thị hoặc làm gì đó với ID của thương hiệu ở đây
                    document.getElementById('selectedIdDisplay').textContent = `ID của thương hiệu đã chọn: ${selectedBrandId}`;
                });
            } else {
                showError('Không thể tải dữ liệu thương hiệu!');
            }
        },
        error: function(xhr) {
