...
function createValueTag(valueObj, attribute, container) {
    // Xóa tất cả các thẻ value-tag cũ trong container
    const existingTags = container.querySelectorAll(".value-tag");
    existingTags.forEach(tag => tag.remove());

    // Tạo thẻ div mới cho value tag
    ...
    removeBtn.onclick = () => {
        tag.remove();
        attribute.values = attribute.values.filter(v => v.id !== valueObj.id);  // Loại bỏ value từ attribute.values
        updateVariants();  // Cập nhật lại variants
    };

    tag.appendChild(removeBtn);
    container.appendChild(tag);

    // Thêm value vào attribute.values sau khi đã tạo tag
    attribute.values.push(valueObj);
}
