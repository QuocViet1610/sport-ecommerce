...
if (!token) {
    ...
    $(".attribute-dropdown").on("select2:select", function (e) {
        // Lấy tất cả các thẻ select trong trang và mảng các giá trị đã chọn
        const selectElements = document.querySelectorAll(".attribute-dropdown");
        const currentValues = [];
    
        selectElements.forEach(select => {
            currentValues.push(select.value);  // Lưu tất cả các giá trị đã chọn từ các thẻ select
        });
    
        // Lấy giá trị mới được chọn từ thẻ select hiện tại
        const selectedValue = $(this).val();
    
        // Kiểm tra xem giá trị mới đã tồn tại trong các thẻ select khác chưa
        if (currentValues.filter(value => value !== "").includes(selectedValue)) {
            // Nếu giá trị đã tồn tại, ngừng hành động và thông báo lỗi
            showError("Thuộc tính đã tồn tại.");
            $(this).val(null).trigger('change');  // Reset lại giá trị và ngừng thay đổi
        } else {
