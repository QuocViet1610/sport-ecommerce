...
function addAttributeValue(attribute, values, valueContainer) {
    ...
    // Kiểm tra nếu thuộc tính đã tồn tại trong attributes
    ...
    // Duyệt qua các giá trị được chọn và thêm vào thuộc tính
    values.forEach(value => {
        // Kiểm tra nếu giá trị chưa tồn tại trong values của existingAttribute
        if (!existingAttribute.values.some(val => val.id === value.id)) {
            // Thêm giá trị vào mảng values của thuộc tính
            existingAttribute.values.push(value);

            // Kiểm tra nếu giá trị đã được chọn trong selectedQuickAttributes
            if (!selectedQuickAttributes.some(attr => attr.id === value.id)) {
                selectedQuickAttributes.push(value);  // Thêm vào mảng selectedQuickAttributes
            }

            // Tạo thẻ value tag cho giá trị
            createValueTag(value, existingAttribute, valueContainer);
        }
    });

    // Cập nhật lại các biến thể
    updateVariants();
}

// Hàm tạo thẻ value-tag cho giá trị thuộc tính
function createValueTag(value, attribute, container) {
    const tag = document.createElement("div");
    tag.classList.add("value-tag");
    tag.textContent = value.name + value.id;

    // Thêm thuộc tính data-id cho mỗi thẻ value-tag
    ...
    // Khi nhấn vào nút xóa, xóa giá trị khỏi mảng và DOM
    removeBtn.onclick = () => {
        tag.remove();
        attribute.values = attribute.values.filter(v => v.id !== value.id);
        selectedQuickAttributes = selectedQuickAttributes.filter(attr => attr.id !== value.id);
        updateVariants();
    };
...
// Hàm xử lý sự kiện khi nhấn nút "Thêm thuộc tính"
