...
const attributeList = document.getElementById('attributeList'); // Lấy phần tử #attributeList từ HTML

// Gửi yêu cầu AJAX để lấy thuộc tính
$.ajax({
    ...
    success: function(response) {
        if (response.code == 200) {
            const attributes = response.data; // Dữ liệu thuộc tính từ API
                // Tạo phần tử dropdown cho mỗi thuộc tính
                const dropdownContainer = document.createElement("attribute-group");
                dropdownContainer.classList.add("dropdown");

                // Tạo nút dropdown
                const dropdownButton = document.createElement("button");
                dropdownButton.classList.add("btn", "btn-success", "dropdown-toggle", "w-100", "dropdow-brand");
                dropdownButton.setAttribute("type", "button");
                dropdownButton.setAttribute("id", "dropdownButtonBrand");
                dropdownButton.setAttribute("data-bs-toggle", "dropdown");
                dropdownButton.setAttribute("aria-expanded", "false");
                dropdownButton.style.backgroundColor = "#F9F9F6";
                dropdownButton.textContent = "Chọn thuộc tính";
            // Tạo dropdown và các thuộc tính từ API
            attributes.forEach(attribute => {

                // Tạo menu dropdown
                const dropdownMenu = document.createElement("ul");
                dropdownMenu.classList.add("dropdown-menu", "w-100");
                dropdownMenu.setAttribute("id", "myDropdownBrand");

                // Thêm input tìm kiếm vào menu
                const searchInput = document.createElement("li");
                searchInput.innerHTML = '<input type="text" class="form-control" id="myInput" placeholder="Search..." onkeyup="filterFunction()">';
                dropdownMenu.appendChild(searchInput);

                // Thêm thuộc tính vào dropdown
                const dropdownItem = document.createElement("li");
                const link = document.createElement("a");
                link.classList.add("dropdown-item");
                link.setAttribute("href", "#");
                link.setAttribute("data-id", attribute.id);
                link.setAttribute("data-name", attribute.name);
                link.textContent = attribute.name;
                ...
                attributeList.appendChild(dropdownContainer); // Thêm dropdown vào attributeList

                // Lắng nghe sự kiện khi người dùng chọn một mục
                dropdownMenu.addEventListener('click', function(e) {
                    if (e.target && e.target.classList.contains('dropdown-item')) {
                        const selectedName = e.target.dataset.name; // Lấy tên thuộc tính đã chọn
                        const selectedId = e.target.dataset.id; // Lấy ID thuộc tính đã chọn

                        // Cập nhật nút dropdown với tên thuộc tính đã chọn
                        dropdownButton.textContent = selectedName;

                        // Lưu ID vào một biến toàn cục
                        console.log('ID của thuộc tính đã chọn:', selectedId);

                        // Bạn có thể lưu ID vào một biến global hoặc thực hiện hành động khác với ID
