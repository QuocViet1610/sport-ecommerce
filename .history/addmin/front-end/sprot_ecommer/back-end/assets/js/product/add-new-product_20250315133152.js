...
addAttributeBtn.addEventListener("click", () => {
    ...
    $.ajax({
        ...
        headers: {
            "Authorization": "Bearer " + token,  // Lấy token từ localStorage hoặc cookie
            "Accept": "application/json"
        },
        success: function(response) {
            // Kiểm tra nếu API trả về dữ liệu thành công
            if(response.code == 200) {
                // Lấy dữ liệu trả về từ response
                ...
                // Thêm mặc định cho thẻ select
                const defaultOption = document.createElement("option");
                defaultOption.value = "aa";
                defaultOption.textContent = "Chọn thuộc tính";
                select.appendChild(defaultOption);
    
                // Thêm các option từ dữ liệu API vào thẻ select
                data.forEach(item => {
                    const option = document.createElement("option");
                    option.value = item.id;  // Giả sử mỗi item có thuộc tính 'id'
                    option.textContent = item.name;  // Giả sử mỗi item có thuộc tính 'name'
                    select.appendChild(option);
                });
    
                // Thêm select vào DOM (ví dụ thêm vào body)
        ...
        error: function(xhr) {
            ...
            showError(errorMessage);  // Hiển thị thông báo lỗi nếu có
        }
    });
    ...
    select.classList.add("attribute-dropdown", "js-example-basic-single");
    select.innerHTML = `
        <option value="aa" selected>Chọn thuộc tính</option>
...
document.getElementById("addValueBtn").addEventListener("click", () => {
    ...
    selectElements.forEach(select => {
        currentValues.push(select.value); // Lấy giá trị hiện tại của mỗi select
    });
