...
// Gắn sự kiện cho tất cả các thẻ select có class "attribute-dropdown"
selectElements.forEach(select => {
    select.addEventListener("change", function() {
        // Xử lý khi giá trị của select thay đổi
        console.log(`Giá trị mới của select ${select.id || ''}: `, select.value);
        
        // Thêm logic xử lý tại đây, ví dụ như cập nhật dữ liệu, thông báo, v.v.
    });
});
...
document.getElementById("addValueBtn").addEventListener("click", () => {
    ...
    const currentValues = [];
    selectElements.forEach(select => {
        currentValues.push(select.value + select.); 
    });
...
function openQuickSelectPopup(attribute, valueContainer) {
    // const selectElements = document.querySelectorAll(".attribute-dropdown");
    // let existingAttributeId = false;
    // selectElements.forEach(select => {
    //     console.log(select.value)

    //     if(attribute == select.value){
    //         existingAttributeId = true;
    //     }
    // });
    // if(existingAttributeId) return showError("Thuộc tính đã tồn tại");
