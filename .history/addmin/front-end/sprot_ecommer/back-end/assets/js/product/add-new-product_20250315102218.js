...
$(document).ready(function() {
        ...
        success: function(data) {
            ...
            if(data.code == 200) {
                ...
                $('.dropdown-item').on('click', function() {
                    var selectedBrand = $(this).data('name'); // Lấy tên thương hiệu đã chọn
                    selectedBrandValue = $(this).data('id'); // Lưu id của thương hiệu vào biến
                    
                    $('#dropdownButtonBrand').text(selectedBrand); // Cập nhật nút dropdown với tên thương hiệu
    ...
    var attributeModel = ""; // Biến để lưu giá trị của thể loại đã chọn
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.code == 200) {
                ...
                // Thêm một option "Chọn thể loại" (truyền rỗng)
                $('#category').append('<option value="" selected>Chọn thể loại</option>');
                ...
                // Lắng nghe sự kiện thay đổi của select để lưu giá trị vào biến attributeModel
                $('#category').on('change', function() {
                    attributeModel = $(this).val(); // Lưu giá trị đã chọn vào biến attributeModel
                    console.log("Giá trị đã chọn:", attributeModel); // Kiểm tra giá trị đã chọn
                });
            } else {
