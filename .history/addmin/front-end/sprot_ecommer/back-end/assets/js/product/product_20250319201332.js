...
function renderProductTable(products, page = 1, itemsPerPage = 5) {
    ...
    products.forEach(product => {
        ...
        // Các biến thể sẽ được phân trang
        let variantRows = product.variants.slice(startIndex, endIndex).map((variant, index) => {
            return `
            <tr class="product-variants child-row" data-parent="${product.id}" style="display: table-row;">
                <td colspan="8">
        ...
        // Thêm sản phẩm chính và các biến thể vào bảng
        wrapperRow += variantRows;
        tbody.append(wrapperRow);
    });

    // Phân trang: Hiển thị các nút phân trang
    renderPagination(products, currentPage, itemsPerPage);
}

function renderPagination(products, currentPage, itemsPerPage) {
    let totalVariants = products.reduce((sum, product) => sum + product.variants.length, 0);
    let totalPages = Math.ceil(totalVariants / itemsPerPage);
    ...
    $(".page-link").on('click', function (event) {
        ...
        let selectedPage = $(this).data("page");
        renderProductTable(products, selectedPage);
    });
}
