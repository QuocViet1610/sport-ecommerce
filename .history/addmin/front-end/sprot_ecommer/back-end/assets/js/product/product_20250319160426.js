            ...
            product.variants.forEach((variant, index) => {
                wrapperRow += `
                    <tr class="product-variants child-row" data-parent="${product.id}" style="display: none; border: 1px solid #000 !important;">
                        <td colspan="8" style="border-left: 1px solid green; border-right: 1px solid green; border-bottom: none;">
                ...
                if (index === product.variants.length - 1) {
                 ...
                 <tr class="product-variants child-row" data-parent="${product.id}" style="display: none; border-bottom: 1px solid green ; border-left: 1px solid green; border-right: 1px solid green;">
                            <td colspan="8" style="text-align: center; font-weight: bold;  border-bottom: 1px solid green ; border-left: 1px solid green; border-right: 1px solid green; margin-bottom:7px !important">
                            </td>
                        </tr>`
                }
            });
...
$(document).ready(function () {
    // Tích chọn tất cả dòng khi click vào checkbox ở header
    $("#selectAll").on('change', function () {
        let isChecked = $(this).prop('checked');
        
        // Tích chọn tất cả checkbox con (bao gồm cả sản phẩm và biến thể)
        $(".select-product, .select-variant").prop('checked', isChecked);
    });

    // Tích chọn sản phẩm cha khi tích chọn sản phẩm con
    $(".select-variant").on('change', function () {
        ...
        // Tích chọn sản phẩm cha nếu tất cả biến thể đã được chọn
        $(`.select-product[data-id="${parentId}"]`).prop('checked', allVariantsChecked);
    });

    // Tích chọn tất cả biến thể khi tích chọn sản phẩm cha
    $(".select-product").on('change', function () {
        let productId = $(this).data('id');
        let isChecked = $(this).prop('checked');

        // Tích chọn hoặc bỏ tích tất cả biến thể của sản phẩm
