    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                ...
                // Đảm bảo bạn đang chọn đúng phần tử select
                ...
                select.addEventListener('change', function() {
                    const selectedId = this.value; // Lấy ID của thuộc tính đã chọn
                    console.log("ID của thuộc tính đã chọn:", selectedId); // In ID ra console
    
                    // Hiển thị ID trong giao diện, ví dụ trong phần tử div
                    document.getElementById('selectedIdDisplay').textContent = `ID được chọn: ${selectedId}`;
                });
            } else {
