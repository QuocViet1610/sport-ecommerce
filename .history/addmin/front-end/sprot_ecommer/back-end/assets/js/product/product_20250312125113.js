...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        ...
        if (clickedRow) {
            ...
            if (isVisible) {
                // Nếu đang mở -> Ẩn biến thể, xóa border & màu nền
                variantRows.forEach(row => row.style.display = "none");
                wrapperRow.classList.remove("active-border", "highlight-green");
            } else {
                // Nếu đang đóng -> Hiển thị biến thể, thêm border & màu nền
                variantRows.forEach(row => row.style.display = "table-row");
                wrapperRow.classList.add("active-border", "highlight-green");
            }
        }
    });
});
