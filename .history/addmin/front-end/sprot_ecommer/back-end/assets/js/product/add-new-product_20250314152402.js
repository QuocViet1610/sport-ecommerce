...
$(document).ready(function() {
    ...
    $.ajax({
     ...
     $.ajax({
        url: "http://localhost:8080/category", // Đường dẫn API
        type: "GET",
        contentType: "application/json",
        headers: {
            "Authorization": "Bearer " + token,
        ...
        success: function(response) {
            ...
            if (response.data && response.data.length > 0) {
                // Chuyển danh sách thành object map theo parentId
                let categoryMap = {};
                ...
                // Gọi hàm để render các danh mục
                renderCategoryOptions(categoryMap, null, 0, categorySelect);
            } else {
    ...
    function renderCategoryOptions(categoryMap, parentId, level, categorySelect) {
        ...
        categoryMap[parentId].forEach(category => {
            ...
            renderCategoryOptions(categoryMap, category.name, level + 1, categorySelect);
        });
    }
});
