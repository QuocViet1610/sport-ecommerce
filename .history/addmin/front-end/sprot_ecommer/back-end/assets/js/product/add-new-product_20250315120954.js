    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                ...
                const dropdownMenu = document.getElementById('myDropdown'); // Lấy menu dropdown
    
                // Tạo một phần tử dropdown mặc định
                let optionsHTML = '<li><input type="text" class="form-control" id="myInput" placeholder="Search..." onkeyup="filterFunction()"></li>';
    
                // Duyệt qua dữ liệu trả về từ API và thêm từng mục vào dropdown
                attributes.forEach(attribute => {
                    optionsHTML += `<li><a class="dropdown-item" href="#" data-id="${attribute.id}" data-name="${attribute.name}">${attribute.name}</a></li>`;
                });
    
                // Cập nhật dropdown với các tùy chọn động
                ...
                $('#myDropdown').on('click', '.dropdown-item', function() {
                    const selectedName = $(this).data('name'); // Lấy tên thuộc tính đã chọn
                    const selectedId = $(this).data('id'); // Lấy ID thuộc tính đã chọn
    
                    // Cập nhật nút dropdown với tên thuộc tính đã chọn
                    $('#dropdownButton').text(selectedName);
    
                    // Lưu ID vào một biến hoặc làm gì đó với ID
                    console.log('ID của thuộc tính đã chọn:', selectedId);
    
                    // Bạn có thể lưu ID vào một biến toàn cục hoặc làm gì đó với nó
                    // Ví dụ: Lưu vào biến global
