    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.code == 200) {
                ...
                // Lắng nghe sự kiện thay đổi của dropdown để cập nhật nút trong modal
                $('#category').on('change', function() {
                    var selectedCategory = $(this).find("option:selected").text(); // Lấy tên thể loại đã chọn
                    $('#dropdownButtonModal').text(selectedCategory); // Cập nhật nút với tên thể loại
                });
            } else {
...
$.ajax({
    ...
    success: function(response) {
        ...
        if(response.code == 200) {
            ...
            parents.forEach(function(parent) {
                console.log("Parent item:", parent); // Debug: kiểm tra đối tượng parent
                console.log("Parent name:", parent.name); // Debug: kiểm tra thuộc tính name
                var parentItem = $('<div>', {
                    class: 'dropdown-item',
                    text: parent.name,
                    click: function() {
