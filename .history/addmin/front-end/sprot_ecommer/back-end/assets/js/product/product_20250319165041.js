...
$(document).ready(function () {
    // Khi checkbox của thẻ cha được chọn hoặc bỏ chọn
    $(".select-product").on('change', function () {
        ...
        // Cập nhật tất cả các checkbox của thẻ con (biến thể) tương ứng với sản phẩm cha
        $(`.select-variant[data-parent="${productId}"]`).prop('checked', isChecked);
    });

    // Khi checkbox của thẻ con được chọn hoặc bỏ chọn
    $(".select-variant").on('change', function () {
        let parentId = $(this).data('parent');  // Lấy ID của sản phẩm cha
        let allVariantsChecked = $(`.select-variant[data-parent="${parentId}"]:checked`).length === $(`.select-variant[data-parent="${parentId}"]`).length;

        // Tích chọn sản phẩm cha nếu tất cả các biến thể đã được chọn
        $(`.select-product[data-id="${parentId}"]`).prop('checked', allVariantsChecked);
    });
...
document.addEventListener("DOMContentLoaded", function () {
    let isCheckboxChanging = false; // Flag để kiểm tra xem người dùng có đang thay đổi checkbox không

    // Lắng nghe sự kiện thay đổi checkbox của sản phẩm cha
    $(".select-product").on('change', function () {
        isCheckboxChanging = true; // Đánh dấu rằng checkbox đang được thay đổi
        console.log("hello");
        ...
        // Ngừng xử lý việc hiển thị/ẩn biến thể
        setTimeout(() => {
            isCheckboxChanging = false; // Sau khi xử lý xong, cho phép hiển thị/ẩn biến thể lại
        }, 100);
    });

    // Lắng nghe sự kiện click vào dòng cha để hiển thị/ẩn các biến thể
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        if (isCheckboxChanging) {
            return; // Nếu người dùng thay đổi checkbox, ngừng xử lý click
        }
