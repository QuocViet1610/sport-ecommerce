...
if (!token) {
    ...
    console.log(selectElements);  // Kiểm tra xem đã lấy đúng các phần tử chưa
    
        // Gắn sự kiện với select2 khi nó được khởi tạo
        $(".attribute-dropdown").on("select2:select", function (e) {
            console.log(`Giá trị mới của select ${this.id || ''}: `, $(this).val());
        });
