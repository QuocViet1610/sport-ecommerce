...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        ...
        if (clickedRow) {
            ...
            // Kiểm tra trạng thái hiện tại của các variantRow
            let isVisible = [...variantRows].some(row => row.style.display === "table-row");

            // Nếu có một variant row đang hiển thị, ẩn tất cả variant rows
            if (isVisible) {
            ...
            // Hiển thị variant row được bấm vào và ẩn các hàng khác
            let targetVariantRow = clickedRow.closest(".parent-row").nextElementSibling; // Giả sử variant nằm liền kề
            if (targetVariantRow && targetVariantRow.classList.contains("product-variants")) {
                targetVariantRow.style.display = "table-row"; // Hiển thị dòng biến thể
