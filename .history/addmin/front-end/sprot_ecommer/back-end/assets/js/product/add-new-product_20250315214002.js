...
// Mảng lưu các id của các giá trị đã chọn


let selectedTagIds = [];  // Mảng lưu các id của tags đã chọn
let selectedAttributes = [];  // Mảng lưu các attributes đã chọn
...
function openQuickSelectPopup(attribute, valueContainer) {
    ...
    selectedAttributes = [];  // Đặt lại mảng khi mở popup
    let token = localStorage.getItem("authToken");
        ...
        success: function(response) {
            if (response.code == "200" && response.data) {
                ...
                values.forEach(valueObj => {
                    ...
                    if (selectedAttributes.some(v => v.id === valueObj.id)) {
                    ...
                    option.onclick = () => {
                        ...
                        if (option.classList.contains("selected")) {
                            selectedAttributes.push(valueObj); // Thêm vào danh sách selected
                            option.style.backgroundColor = "#4CAF50";  // Màu nền xanh khi được chọn
                            option.style.color = "white";  // Màu chữ trắng
                        } else {
                            selectedAttributes = selectedAttributes.filter(attr => attr.id !== valueObj.id);
    ...
    confirmPopup.onclick = () => {
        addAttributeValue(attribute, selectedAttributes, valueContainer);
...
function createValueTag(valueObj, attribute, container) {
    ...
    removeBtn.onclick = () => {
        ...
        // Khi xóa, loại bỏ id khỏi mảng selectedAttributes
        selectedAttributes = selectedAttributes.filter(id => id !== valueObj.id);
    };
