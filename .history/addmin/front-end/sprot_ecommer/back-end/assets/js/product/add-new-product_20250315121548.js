    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                const attributes = response.data; // Lấy dữ liệu thuộc tính từ API
                console.log(attributes); // Debug: Kiểm tra dữ liệu thuộc tính
                ...
                // Duyệt qua dữ liệu trả về từ API và tạo các tùy chọn
                attributes.forEach(attribute => {
                    const option = document.createElement("option");
                    option.value = attribute.id;  // Giả sử rằng API trả về `id` của thuộc tính
                    option.textContent = attribute.name;  // Hiển thị tên thuộc tính
                    select.appendChild(option);
                });
    
                // Thêm select vào DOM
                document.body.appendChild(select);  // Thêm phần tử select vào body hoặc vào vị trí mong muốn trong DOM
    
                // Kích hoạt Select2 cho select dropdown
                $(select).select2();
            } else {
