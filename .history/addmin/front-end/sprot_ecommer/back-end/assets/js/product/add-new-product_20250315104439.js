...
$(document).ready(function() {
    ...
    var attributeModel = ""; // Biến để lưu ID thể loại đã chọn
    ...
    $.ajax({
        ...
        success: function(response) {
            if(response.code == 200) {
                ...
                // Lắng nghe sự kiện thay đổi của dropdown để cập nhật nút trong modal và lưu ID vào biến attributeModel
                $('#category').on('change', function() {
                    var selectedCategory = $(this).find("option:selected").text(); // Lấy tên thể loại đã chọn
                    var selectedCategoryId = $(this).val(); // Lấy ID của thể loại đã chọn
    
                    $('#dropdownButtonModal').text(selectedCategory); // Cập nhật nút với tên thể loại
                    attributeModel = selectedCategoryId; // Lưu ID vào biến attributeModel
    
                    console.log("ID thể loại đã chọn:", attributeModel); // Kiểm tra ID đã chọn
                });
            } else {
