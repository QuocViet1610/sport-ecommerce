...
function renderProductTable(products) {
    ...
    products.forEach(product => {
        ...
        // Nếu sản phẩm có biến thể, hiển thị dòng con
        if (product.variants.length > 0) {
            let variantRows = `
                <tr class="product-variants child-row" data-parent="${product.id}">
                    <td colspan="8">
            ...
            tbody.append(variantRows);
        }
    });
}

document.addEventListener("DOMContentLoaded", function () {
    // Dùng event delegation để lắng nghe click trên tbody (vì bảng có thể cập nhật sau)
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        let clickedRow = event.target.closest(".parent-row"); // Tìm hàng cha được click
        ...
        if (clickedRow) {
            ...
            if (variantRows.length > 0) {
                ...
                // Toggle border cho dòng cha
                if (clickedRow.classList.contains("highlight-border")) {
                    clickedRow.classList.remove("highlight-border");
                } else {
                    // Xóa border các hàng khác (đảm bảo chỉ có 1 hàng được highlight)
                    document.querySelectorAll(".parent-row").forEach(row => row.classList.remove("highlight-border"));
                    clickedRow.classList.add("highlight-border");
                }
            }
        }
    });

    // Ngăn chặn click vào icon sửa, xóa mở dòng con
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
