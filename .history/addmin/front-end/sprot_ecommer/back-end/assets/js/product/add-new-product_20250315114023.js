    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(response.code == 200) {
                ...
                // Thêm sự kiện change để lưu vào mảng `attributes` khi chọn
                select.addEventListener('change', function() {
                    const selectedValue = select.value;  // Lấy giá trị của mục được chọn
                    const selectedAttribute = attributesData.find(attr => attr.id === selectedValue);  // Tìm thuộc tính tương ứng
    
                    if (selectedAttribute) {
                        attributes = [selectedAttribute];  // Lưu thuộc tính vào mảng `attributes`
                        console.log(attributes);  // In mảng `attributes` ra console để kiểm tra
                    }
                });
