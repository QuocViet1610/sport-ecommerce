...
$.ajax({
    ...
    success: function(response) {
        if(response.code == 200) {
            ...
            // Hàm đệ quy để thêm các mục con cho mỗi mục cha
            function addChildrenRecursively(parentId, level, dropdownId) {
                if (children[parentId]) {
                    children[parentId].forEach(function(child) {
                        ...
                        var childItem = $('<div>', {
                            ...
                            html: '&nbsp;&nbsp;&nbsp;&nbsp;'.repeat(level) + child.name,
                            click: function() {
                                selectItem(child.name, dropdownId); // Khi chọn mục con
                            }
                        });

                        // Thêm mục con vào dropdown
                        $('#' + dropdownId).append(childItem);

                        // Gọi đệ quy để thêm các mục con cấp sâu hơn
                        addChildrenRecursively(child.id, level + 1, dropdownId);
                    });
                }
            }
            ...
            parents.forEach(function(parent) {
                var parentItem = $('<div>', {
                    ...
                    text: parent.name,
                    click: function() {
                        selectItem(parent.name, "myDropdown"); // Khi chọn mục cha
                    }
                });

                // Thêm mục cha vào dropdown
                $('#myDropdown').append(parentItem);

                // Kiểm tra các mục con của cha hiện tại
                addChildrenRecursively(parent.id, 1, "myDropdown");
                
                // Thêm vào dropdown thứ hai nếu cần
                var parentItem2 = $('<div>', {
                    class: 'dropdown-item',
                    text: parent.name,
                    click: function() {
                        selectItem(parent.name, "myDropdown2"); // Khi chọn mục cha
                    }
                });

                $('#myDropdown2').append(parentItem2);
                addChildrenRecursively(parent.id, 1, "myDropdown2");
            });
        } else {
            alert('Lỗi: ' + response.message);
        }
    ...
    error: function(xhr, status, error) {
        console.error("Lỗi khi lấy dữ liệu thể loại:", xhr.responseText || error);
        let categoryDropdown = $("#myDropdown, #myDropdown2");
        categoryDropdown.empty();
        categoryDropdown.append('<div class="dropdown-item" disabled>Không thể tải thể loại</div>');
    }
});

function selectItem(itemText, dropdownId) {
    document.getElementById(dropdownId === "myDropdown" ? "dropdownButton" : "dropdownButton2").textContent = itemText;
    var dropdown = new bootstrap.Dropdown(document.getElementById(dropdownId === "myDropdown" ? "dropdownButton" : "dropdownButton2"));
    dropdown.hide();
}
