...
let searchRequest = {
  searchText: "" // Mặc định không có tìm kiếm
};

let typingTimer; // Biến để theo dõi thời gian gõ phím
const typingDelay = 500; // Thời gian chờ (ms) trước khi gửi yêu cầu (tránh spam API)

// Khi người dùng nhập vào ô tìm kiếm
$("#searchInput").on("input", function () {
  clearTimeout(typingTimer); // Xóa timer trước đó

  let searchText = $(this).val().trim(); // Lấy giá trị từ ô nhập
  searchRequest.searchText = searchText; // Cập nhật giá trị tìm kiếm

  // Đợi `typingDelay` ms trước khi gọi API
  ...
  function fetchData(page) {
      ...
      $.ajax({
          url: `http://localhost:8080/category/search?page=${page - 1}&size=${pageSize}&sort=id.asc`, 
          type: "POST",
          ...
          error: function (xhr, status, error) {
            ...
            // Nếu API trả về JSON chứa `message`, lấy thông báo lỗi từ đó
            ...
            // Hiển thị lỗi trên UI (nếu có `showError`)
