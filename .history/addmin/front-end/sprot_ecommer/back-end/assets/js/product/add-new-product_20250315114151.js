    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if(response.code == 200) {
                const attributesData = response.data; // Dữ liệu thuộc tính từ API
                select.classList.add("attribute-dropdown", "js-example-basic-single");
                let optionsHTML = '<option value="" selected>Chọn thuộc tính</option>';
                ...
                // Cập nhật select với các tùy chọn động
                select.innerHTML = optionsHTML;
    
                // Lắng nghe sự kiện chọn thuộc tính
                select.addEventListener('change', function() {
                    const selectedId = this.value; // Lấy ID thuộc tính đã chọn
                    
                    // Kiểm tra xem ID đã tồn tại trong mảng attributes chưa, nếu chưa thì thêm vào
                    if (selectedId && !attributes.includes(selectedId)) {
                        attributes.push(selectedId);
                    }
    
                    // Lọc các phần tử trùng lặp trong mảng attributes
                    attributes = [...new Set(attributes)]; // Sử dụng Set để loại bỏ trùng lặp
                    console.log("Các thuộc tính đã chọn:", attributes); // In mảng các ID thuộc tính đã chọn
                });
