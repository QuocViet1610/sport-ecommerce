...
function openQuickSelectPopup(attribute, valueContainer) {
        ...
        success: function(response) {
            if (response.code == "200" && response.data) {
                ...
                values.forEach(valueObj => {
                    ...
                    selectedAttributeList.push(valueObj.id)
    ...
    confirmPopup.onclick = () => {
        ...
        const allTags = document.querySelectorAll("#attributeList .value-tag");
        allTags.forEach(tag => tag.remove());  // Loại bỏ tất cả thẻ tag trong attributeList
    
    
        // Gọi hàm addAttributeValue để thêm các thẻ mới vào
        addAttributeValue(attribute, selectedQuickAttributes, valueContainer);
        console.log("select "+selectedQuickAttributes)
    ...
    values.forEach(value => {
        if (!existingAttribute.values.includes(value)) {
            existingAttribute.values.push(value);
                ...
                if (selectedQuickAttributes.some(attr => attr.id === value.id)) {
...
$(document).ready(function () {
    ...
    selectElements.forEach(select => {
        ...
        attributeGroups.forEach(group => {
            ...
            // Nếu cần lấy ID thực sự thay vì text, bạn có thể cần thêm logic để trích xuất ID từ data-* attribute
            // Ví dụ: Từ "Màu sắc 2" trích xuất ra ID, nếu có
            console.log("attributeText:", attributeText);
            
            // Để lấy ID thực (giả sử ID được lưu trong data attribute hoặc có thể trích xuất từ title)
            let attributeId = null;
            // Nếu ID được lưu trong data attribute
            if (attributeIdElement && attributeIdElement.hasAttribute("data-id")) {
                attributeId = parseInt(attributeIdElement.getAttribute("data-id"));
            }
            // Hoặc nếu ID có thể trích xuất từ text (ví dụ: "Màu sắc 2" -> ID: 2)
            else if (attributeText && attributeText.match(/\d+$/)) {
                attributeId = parseInt(attributeText.match(/\d+$/)[0]);
            }
            
            // Lấy các giá trị valueId từ .value-tag trong .value-container
            let valueIds = [];
            group.querySelectorAll(".value-container .value-tag").forEach(valueTag => {
                let valueId = valueTag.id.replace("value-tag-", ""); // Lấy ID và loại bỏ phần 'value-tag-'
                console.log(valueId);
                if (valueId) {
                    valueIds.push(parseInt(valueId));  // Thêm valueId vào mảng valueIds
                }
            });
            console.log(valueIds);  // Kiểm tra giá trị của mảng valueIds
