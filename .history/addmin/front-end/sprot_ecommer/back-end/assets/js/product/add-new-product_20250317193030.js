...
// Lấy hình ảnh từ input file và thêm vào FormData
            ...
            let fileCount = 0;
            let imagesArray = []; // Mảng để chứa tất cả ảnh
            
            selectedImages.forEach((imgObj, index) => {
                imagesArray.push(imgObj.file);  // Đưa file vào mảng thay vì FormData trực tiếp
                console.log(`📸 Ảnh ${index + 1}:`, imgObj.file.name);
                fileCount++;
            });
            formData.append("image", new Blob(imagesArray), "images.zip");
