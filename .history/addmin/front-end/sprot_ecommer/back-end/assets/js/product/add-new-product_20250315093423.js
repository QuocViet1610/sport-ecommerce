...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data && data.length > 0) {
                ...
                // Debug: Kiểm tra dropdownMenu trước khi xóa các phần tử
                console.log("Dropdown Menu trước khi xóa:", dropdownMenu);
    
                dropdownMenu.empty();
    
                // Debug: Kiểm tra dropdownMenu sau khi đã xóa
                console.log("Dropdown Menu sau khi xóa:", dropdownMenu);
    
                dropdownMenu.append('<li><input type="text" class="form-control" id="myInput" placeholder="Search..." onkeyup="filterFunction()"></li>');
                
                // Debug: Kiểm tra từng mục được thêm vào dropdown
                $.each(data, function(index, item) {
                    console.log("Thêm mục vào dropdown:", item.name); // Kiểm tra tên của thương hiệu
                    dropdownMenu.append('<li><a class="dropdown-item" href="#">' + item.name + '</a></li>');
                });
            } else {
                // Nếu không có dữ liệu
                console.log("Không có dữ liệu thương hiệu.");
                $('#myDropdownBrand').append('<li><span>không có dữ liệu</span></li>');
            }
        },
        error: function(xhr, status, error) {
            console.error("Lỗi khi lấy dữ liệu thương hiệu:", xhr.responseText || error);
    ...
    // Hàm lọc các mục trong dropdown khi người dùng nhập vào thanh tìm kiếm
    function filterFunction() {
        const input = document.getElementById("myInput");
        const filter = input.value.toUpperCase();
        const div = document.getElementById("myDropdownBrand");
    ...
    success: function(response) {
        ...
        if(response.code == 200) {
            ...
            function addChildrenRecursively(parentId, level) {
                if (children[parentId]) {
                    children[parentId].forEach(function(child) {
                        ...
                        // Thêm mục con vào dropdown ngoài modal
                        $('#myDropdown').append(childItem);

                        // Thêm mục con vào dropdown trong modal
                        $('#myDropdownModal').append(childItem.clone());

                        // Gọi đệ quy để thêm các mục con cấp sâu hơn
                        addChildrenRecursively(child.id, level + 1);
                    });
                }
            }
            ...
            parents.forEach(function(parent) {
                ...
                $('#myDropdownModal').append(parentItem.clone());
...
// Hàm selectItem được gọi khi chọn một mục
function selectItem(id) {
    // Gắn ID thể loại được chọn vào biến value
    value = id;
    console.log("ID thể loại được chọn:", value); // In ra giá trị ID của thể loại được chọn
}
...
window.filterFunction = function() {
    ...
    ul = document.getElementById("myDropdownBrand");
