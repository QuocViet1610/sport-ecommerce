...
// Replace the global selectedQuickAttributes array with a map
let selectedAttributeValues = new Map(); // Map attribute IDs to their selected values

// In the openQuickSelectPopup function, modify to use the map
function openQuickSelectPopup(attribute, valueContainer) {
    ...
    if (!selectedAttributeValues.has(attribute)) {
        selectedAttributeValues.set(attribute, []);
    }
    
    // Get the current selected values for this specific attribute
    let currentSelectedValues = selectedAttributeValues.get(attribute);
        ...
        success: function(response) {
            if (response.code == "200" && response.data) {
                ...
                values.forEach(valueObj => {
                    ...
                    // Check if this value is already selected for this attribute
                    const isSelected = currentSelectedValues.some(val => val.id === valueObj.id);
                    if (isSelected) {
                    ...
                    option.onclick = () => {
                        ...
                        if (option.classList.contains("selected")) {
                            // Add to this attribute's selected values if not already there
                            if (!currentSelectedValues.some(val => val.id === valueObj.id)) {
                                currentSelectedValues.push(valueObj);
                            }
                        } else {
                            // Remove from this attribute's selected values
                            currentSelectedValues = currentSelectedValues.filter(val => val.id !== valueObj.id);
                            selectedAttributeValues.set(attribute, currentSelectedValues);
                        }
                    };
    ...
    confirmPopup.onclick = () => {
        // Clear existing value tags in this container
        valueContainer.innerHTML = "";
        
        // Add the selected values for this attribute
        addAttributeValue(attribute, currentSelectedValues, valueContainer);
...
// Modify the addAttributeValue function
function addAttributeValue(attribute, values, valueContainer) {
    ...
    if (!existingAttribute) {
        ...
        // Clear existing values for this attribute
        existingAttribute.values = [];
    }
    
    // Add all selected values to this attribute
...
// Update the createValueTag function to show only the name
function createValueTag(valueObj, attribute, container) {
    const tag = document.createElement("div");
    tag.classList.add("value-tag");
    
    // Add data attribute for the ID (better than using the element ID)
    tag.setAttribute("data-id", valueObj.id);
    
    // Show only the name, not the ID
    ...
    removeBtn.onclick = () => {
        ...
        // Also remove from the selectedAttributeValues map
        let attrValues = selectedAttributeValues.get(attribute.name);
        if (attrValues) {
            attrValues = attrValues.filter(val => val.id !== valueObj.id);
            selectedAttributeValues.set(attribute.name, attrValues);
        }
...
//     let dropdown = $(this).find(".attribute-dropdown");
//     if (dropdown.length > 0) {
//         let attributeId = dropdown.val();
//         console.log(attributeId);
//     } else {
//         console.log("Không có .attribute-dropdown trong nhóm này.");
//     }
//     // Lấy ID của thuộc tính từ thẻ select trong mỗi .attribute-group
//     let attributeId = $(this).find(".attribute-dropdown").val();
//     console.log(attributeId)
//     // Mảng lưu trữ các giá trị thuộc tính
//     let attributeValues = [];

//     // Lấy tất cả các giá trị của .value-tag bên trong .value-container trong mỗi .attribute-group
//     $(this).find(".value-container .value-tag").each(function () {
//         // Lấy giá trị data-id của mỗi thẻ .value-tag
//         attributeValues.push(parseInt($(this).attr("data-id")));
//     });

//     // Kiểm tra nếu có attributeId hợp lệ và có giá trị thuộc tính trong attributeValues
//     if (attributeId && attributeValues.length > 0) {
//         // Thêm đối tượng chứa attributeId và attributeValueIds vào mảng attributeCreatRequests
