...
addAttributeBtn.addEventListener("click", () => {
    ...
    div.innerHTML = `
      <button class="btn btn-success dropdown-toggle w-100" type="button" id="dropdownButton" data-bs-toggle="dropdownAttributte" aria-expanded="false">
        Chọn thể loại
      </button>
      <ul class="dropdown-menu w-100" id="myDropdownAttributte">
        <li>
          <input type="text" class="form-control" id="myInput" placeholder="Search..." onkeyup="filterFunction()">
        </li>
        <!-- Mục dropdown sẽ được thêm vào đây -->
      </ul>
    `;

// AJAX để lấy dữ liệu từ API
$.ajax({
    url: "http://localhost:8080/attribute/all",  
    type: "GET",
    contentType: "application/json",
    headers: {
        "Authorization": "Bearer " + token,  // Giả sử bạn đã có token từ login hoặc session
        "Accept": "application/json"
    },
    success: function(response) {
        if(response.code == 200) {
            // Duyệt qua dữ liệu trả về và thêm các mục vào dropdown
            const dropdownMenu = document.getElementById("myDropdownAttributte");
            response.data.forEach(item => {
                // Tạo một phần tử <li> mới cho mỗi mục
                const li = document.createElement("li");
                const anchor = document.createElement("a");
                anchor.classList.add("dropdown-item");
                anchor.textContent = item.name;  // Giả sử mỗi item có thuộc tính `name`
                console.log(item.name)
                // Khi người dùng chọn, gọi hàm selectItem
    ...
    error: function(xhr) {
        ...
        showError(errorMessage);  // Hàm showError sẽ thông báo lỗi nếu cần
    }
});
