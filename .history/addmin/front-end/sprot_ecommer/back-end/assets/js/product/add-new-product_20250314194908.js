...
const createAttributeButton = document.getElementById('createAttribute');
const saveAttributeButton = document.getElementById('saveAttribute');

// Mở Modal khi nhấn vào nút "+"
createAttributeButton.addEventListener('click', function() {
  attributeModal.show();
});

// Lưu thuộc tính và đóng Modal
saveAttributeButton.addEventListener('click', function(event) {
  ...
  // Kiểm tra nếu tên và giá trị thuộc tính không rỗng
  if (attributeName && attributeValue) {
    alert(`Đã thêm thuộc tính: ${attributeName} với giá trị: ${attributeValue}`);
    
    // Lưu giá trị vào localStorage hoặc thực hiện hành động khác
    // localStorage.setItem('newAttribute', JSON.stringify({ name: attributeName, value: attributeValue }));
