...
function renderVariants(variants) {
    ...
    // Bước 1: Nhóm các biến thể theo tên và ID
    let groupedVariants = {};
    variants.forEach(variant => {
        const groupKey = variant.map(v => `${v.name} (ID: ${v.id})`).join(" - ");
    ...
    // Bước 2: Duyệt qua các nhóm và render
    Object.keys(groupedVariants).forEach(groupKey => {
        const group = groupedVariants[groupKey];
        
        // Tạo một dòng mới cho nhóm biến thể
        const row = document.createElement("tr");

        // Giá trị đầu tiên (Tên và ID biến thể)
        const firstColumn = document.createElement("td");
        firstColumn.setAttribute("rowspan", group.length); // Gộp ô cho các biến thể cùng nhóm
        firstColumn.textContent = groupKey; // Tên và ID của biến thể
        row.appendChild(firstColumn);

        // Duyệt qua từng biến thể trong nhóm
        group.forEach((variant, index) => {
            if (index !== 0) {
                // Tạo một dòng mới cho các biến thể tiếp theo trong nhóm
                const row = document.createElement("tr");
            }

            // Cột ID ẩn
            const variantIdCell = document.createElement("td");
            variantIdCell.style.display = "none"; // Ẩn cột ID
            variantIdCell.classList.add("variant-id");
            variantIdCell.textContent = variant.map(v => v.id).join(", ");
            row.appendChild(variantIdCell);

            // Các cột giá trị (Giá nhập, giá bán, số lượng tồn kho)
            row.innerHTML += `
