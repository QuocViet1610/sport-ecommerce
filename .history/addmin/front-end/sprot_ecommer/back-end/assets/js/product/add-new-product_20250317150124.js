            ...
            if (response.code === "200" && response.data) {
                ...
                values.forEach(valueObj => {
                    ...
                    if (attributeSelected.includes(valueObj.id)) {
                        ...
                        option.style.opacity = "0.5"; // Làm mờ
...
function createValueTag(valueObj, attribute, container) {
    ...
    removeBtn.onclick = () => {
        ...
        attributeSelected = attributeSelected.filter(id => id !== valueObj.id); // Cập nhật danh sách giá trị đã chọn

        // Cập nhật popup để cho phép chọn lại giá trị
        updatePopupValues();
    };
...
function updatePopupValues() {
    document.querySelectorAll(".attribute-option").forEach(option => {
        ...
        if (!attributeSelected.includes(valueId)) {
            option.classList.remove("disabled"); // Bỏ class vô hiệu hóa
            option.style.opacity = "1"; // Hiện rõ lại
            option.style.pointerEvents = "auto"; // Cho phép click lại
        }
    });
}
