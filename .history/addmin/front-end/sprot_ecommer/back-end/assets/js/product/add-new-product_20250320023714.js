    ...
    `;
        ...
        row.querySelectorAll('.price-stock').forEach(function(input) {
            input.addEventListener("input", function(e) {
                ...
                // Loại bỏ mọi ký tự không phải số nguyên và dấu phẩy
                value = value.replace(/[^0-9,]/g, '');  // Chỉ cho phép các ký tự số nguyên và dấu phẩy
        
                // Chuyển giá trị thành số nguyên
                let numValue = parseInt(value.replace(/,/g, ''), 10);  // Loại bỏ dấu phẩy trước khi chuyển thành số
