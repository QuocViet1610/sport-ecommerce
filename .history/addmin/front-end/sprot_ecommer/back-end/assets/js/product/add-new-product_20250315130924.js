...
let attributeCount = 0;
    ...
    document.getElementById('addAttributeBtn').addEventListener('click', function() {
        ...
        // Tạo dropdown cho thuộc tính mới
        const newAttribute = document.createElement('div');
        newAttribute.classList.add('attribute-item');
        newAttribute.innerHTML = `
            <div class="attribute-name">
                <input type="text" class="form-control" placeholder="Thuộc tính ${attributeCount}" />
                <button class="btn btn-success select-value-btn" onclick="openPopup('${attributeCount}')">Chọn nhanh</button>
            </div>
            <div class="selected-values" id="selectedValues${attributeCount}">
                <!-- Giá trị đã chọn sẽ hiển thị ở đây -->
            </div>
            <button class="remove-attribute-btn" onclick="removeAttribute(${attributeCount})">Xóa</button>
        `;
    ...
    function openPopup(attributeId) {
        document.getElementById('quickSelectPopup').style.display = 'block';

        // Cập nhật các giá trị thuộc tính cho popup
        const values = ['xl', 'dd', 'aa', 'bb']; // Thêm giá trị mẫu của thuộc tính vào đây

        const attributeOptions = document.getElementById('attributeOptions');
        attributeOptions.innerHTML = ''; // Xóa các tùy chọn cũ

        // Thêm các tùy chọn vào popup
    ...
    function selectValue(value, attributeId) {
        const selectedValuesDiv = document.getElementById(`selectedValues${attributeId}`);
        const newSelectedValue = document.createElement('span');
        newSelectedValue.classList.add('selected-value');
        newSelectedValue.textContent = value;

        // Thêm giá trị đã chọn vào danh sách
        selectedValuesDiv.appendChild(newSelectedValue);
    ...
    function removeAttribute(attributeId) {
        const attributeItem = document.querySelector(`#attributeList .attribute-item:nth-child(${attributeId})`);
