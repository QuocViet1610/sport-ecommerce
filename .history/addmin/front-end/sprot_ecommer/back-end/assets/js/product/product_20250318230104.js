    ...
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        ...
        if (clickedRow) {
            ...
            if (isVisible) {
                ...
                // Reset màu sắc border của các div variant-wrapper
                let variantWrappers = document.querySelectorAll(".variant-wrapper");
                variantWrappers.forEach(wrapper => wrapper.style.border = "none");
            } else {
                ...
                // Thay đổi màu sắc border của div variant-wrapper thành xanh lá cây
                let variantWrappers = document.querySelectorAll(`.variant-wrapper[data-parent="${productId}"]`);
                variantWrappers.forEach(wrapper => wrapper.style.border = "2px solid green");
            }
        }
    });
});
