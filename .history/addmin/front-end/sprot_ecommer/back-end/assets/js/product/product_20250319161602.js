...
$(document).ready(function () {
    // Khi checkbox sản phẩm chính được chọn, chọn tất cả biến thể
    $(".select-product").on('change', function () {
        let productId = $(this).data('id');
        let isChecked = $(this).prop('checked');
        
        // Cập nhật tất cả checkbox biến thể của sản phẩm
        $(`.select-variant[data-parent="${productId}"]`).prop('checked', isChecked);
    });

    // Khi checkbox biến thể được chọn, kiểm tra tất cả biến thể của sản phẩm cha
    $(".select-variant").on('change', function () {
        let productId = $(this).data('parent');
        let allChecked = $(`.select-variant[data-parent="${productId}"]:checked`).length === $(`.select-variant[data-parent="${productId}"]`).length;
        
        // Cập nhật checkbox của sản phẩm cha
        $(`.select-product[data-id="${productId}"]`).prop('checked', allChecked);
    });
});
