...
select.classList.add("form-select", "js-example-basic-single");

// Thêm select vào selectContainer
selectContainer.appendChild(select);
document.body.appendChild(selectContainer);  // Thêm selectContainer vào DOM (có thể thay đổi theo vị trí bạn muốn)
    ...
    success: function(response) {
        if (response.code == 200) {
            ...
            attributes.forEach(attribute => {
                ...
                option.dataset.id = attribute.id; // Lưu id vào data-id
                option.dataset.name = attribute.name; // Lưu name vào data-name
                select.appendChild(option);
            });

            // Đảm bảo sự kiện change được gán sau khi dropdown được thêm vào DOM
            select.addEventListener("change", function() {
                const selectedOption = this.options[this.selectedIndex];
                const selectedName = selectedOption.dataset.name; // Lấy tên thuộc tính đã chọn
                const selectedId = selectedOption.dataset.id; // Lấy ID thuộc tính đã chọn
                ...
                // Lưu ID vào biến toàn cục hoặc làm gì đó với nó
