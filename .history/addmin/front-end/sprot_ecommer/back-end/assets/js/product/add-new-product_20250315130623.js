...
let attributeCount = 0;
    ...
    // Khi bấm vào nút thêm thuộc tính
    document.getElementById('addAttributeBtn').addEventListener('click', function() {
        ...
        newAttribute.innerHTML = `
            ...
            <div class="dropdown">
                <button class="btn btn-success dropdown-toggle" type="button" id="dropdownButton${attributeCount}" data-bs-toggle="dropdown" aria-expanded="false">
                    Chọn thuộc tính
                </button>
                <ul class="dropdown-menu" id="dropdownMenu${attributeCount}">
                    <li><a class="dropdown-item" href="#" onclick="openPopup(${attributeCount}, 'Màu sắc')">Màu sắc</a></li>
                    <li><a class="dropdown-item" href="#" onclick="openPopup(${attributeCount}, 'Kích thước')">Kích thước</a></li>
                    <li><a class="dropdown-item" href="#" onclick="openPopup(${attributeCount}, 'Chất liệu')">Chất liệu</a></li>
                </ul>
            </div>
        `;
    ...
    // Mở popup chọn giá trị
    function openPopup(attributeId, attributeType) {
        document.getElementById('quickSelectPopup').style.display = 'block';

        // Cập nhật tiêu đề của popup
        document.querySelector('.popup-content h3').textContent = `Chọn giá trị cho thuộc tính: ${attributeType}`;

        // Tạo các giá trị mẫu cho thuộc tính
        const values = attributeType === 'Màu sắc' ? ['Red', 'Blue', 'Green'] :
                       attributeType === 'Kích thước' ? ['Large', 'Small'] :
                       ['Cotton', 'Leather', 'Polyester']; // Thêm các giá trị cho các thuộc tính khác

        // Xóa các tùy chọn cũ
        const attributeOptions = document.getElementById('attributeOptions');
        attributeOptions.innerHTML = '';

        // Thêm giá trị thuộc tính vào popup
        values.forEach(value => {
            const option = document.createElement('div');
            option.classList.add('attribute-option');
            option.innerHTML = `
                <button class="dropdown-item" onclick="selectValue('${value}', ${attributeId})">${value}</button>
            `;
    ...
    function selectValue(value, attributeId) {
        const selectedButton = document.querySelector(`#attributeList .attribute-item:nth-child(${attributeId}) .dropdown-toggle`);
        selectedButton.innerHTML = `Chọn giá trị: ${value}`;

        // Đóng popup
        document.getElementById('quickSelectPopup').style.display = 'none';
    }
    ...
    // Lọc giá trị thuộc tính trong popup
    document.getElementById('searchInput').addEventListener('keyup', function() {
        const filter = this.value.toLowerCase();
        const options = document.querySelectorAll('.attribute-option');
    ...
    // Sắp xếp giá trị thuộc tính trong popup
    document.getElementById('sortDropdown').addEventListener('change', function() {
        const options = document.querySelectorAll('.attribute-option');
        const sortedOptions = Array.from(options).sort((a, b) => {
            const textA = a.textContent.toLowerCase();
            const textB = b.textContent.toLowerCase();
            return this.value === 'A → Z' ? textA.localeCompare(textB) : textB.localeCompare(textA);
        });

        const attributeOptions = document.getElementById('attributeOptions');
        attributeOptions.innerHTML = ''; // Xóa các tùy chọn cũ
