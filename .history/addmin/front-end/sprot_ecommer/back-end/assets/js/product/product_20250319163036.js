...
$(document).ready(function () {
    // Khi checkbox của thẻ cha được chọn, tất cả checkbox của thẻ con sẽ được chọn
    $(".select-product").on('change', function () {
        let productId = $(this).data('id');
        let isChecked = $(this).prop('checked');

        // Tích chọn hoặc bỏ tích tất cả checkbox biến thể của sản phẩm con
        $(`.select-variant[data-parent="${productId}"]`).prop('checked', isChecked);
    });

    // Khi checkbox của thẻ con được chọn, kiểm tra tất cả các thẻ con có được chọn hết không
