...
function renderProductTable(products) {
    ...
    products.forEach(product => {
        ...
        let wrapperRow = `<div class="wrapper-row" data-id="${product.id}">`;  // Bọc thẻ div cho cả cha và con
        wrapperRow += `<tr class="product-row parent-row" data-id="${product.id}">`;
    ...
    // Đóng nhóm bao bọc
    wrapperRow += `</div>`;  // Đóng thẻ div bọc
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelector("tbody.product-group").addEventListener("click", function (event) {
        ...
        if (clickedRow) {
            ...
            let wrapperRow = clickedRow.closest(".wrapper-row");  // Lấy div bao quanh
            ...
            if (isVisible) {
                ...
                wrapperRow.style.border = "none";  // Reset border
            } else {
                ...
                wrapperRow.style.border = "2px solid green";  // Thêm border xanh lá
            }
        }
    });
});
