                ...
                function addChildrenRecursively(parentId, level) {
                    if (children[parentId]) {
                        children[parentId].forEach(function(child) {
                            // Tạo thẻ span để tạo khoảng cách (thụt vào) theo cấp độ
                            var indent = $('<span>', {
                                text: ' '.repeat(level) // Khoảng cách thụt vào theo cấp độ
                            });
    
                            // Tạo option cho mục con
                            var childOption = $('<option>', {
                                value: child.id,
                                text: indent.text() + child.name,  // Thêm khoảng cách thụt vào
                                style: 'padding-left: ' + (20 * level) + 'px !important;' // Thụt vào theo cấp
                            });
    
                            // Thêm mục con vào select
                            $('#category').append(childOption);
    
                            // Gọi đệ quy để kiểm tra các cấp con tiếp theo
