    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                ...
                data.forEach(item => {
                    // Nếu item.id chưa tồn tại trong currentValues thì mới thêm vào
                    if (!currentValues.includes(item.id.toString())) {  // Chuyển item.id thành chuỗi để so sánh
                        ...
                        selectElements.forEach(select => {
                            select.appendChild(option); // Thêm option vào tất cả các thẻ select
                        });
                    }
                });
