    ...
    success: function(response) {
        if (response.code == 200) {
            ...
            parents.forEach(function(parent) {
                ...
                // Thêm các mục con vào đúng vị trí dưới cha
                if (children[parent.id]) {
                    children[parent.id].forEach(function(child) {
                        var childOption = $('<option>', {
                            value: child.id,
                            text: '— ' + child.name,  // Thêm dấu gạch ngang để thụt vào
                            style: 'padding-left: 20px;' // Đảm bảo mục con thụt vào dưới mục cha
                        });
                        optgroup.append(childOption);
                    });
                }

                // Thêm optgroup vào select
