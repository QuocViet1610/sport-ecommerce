...
// Khi bấm vào nút "Tạo mới", attributepop sẽ hiển thị
$('#createAttribute').on('click', function() {
    $('#attributepop').fadeIn();  // Hiển thị attributepop với hiệu ứng fade in
});

// Khi bấm vào dấu X để đóng attributepop
$('#attributepopClose').on('click', function() {
    $('#attributepop').fadeOut();  // Ẩn attributepop với hiệu ứng fade out
});

// Nếu người dùng bấm ra ngoài attributepop, nó cũng sẽ đóng
$('#attributepop').on('click', function(event) {
    if (event.target === this) {
        $('#attributepop').fadeOut();
    }
});
