                ...
                parents.forEach(function(parent) {
                    ...
                    if (children[parent.id]) {
                        ...
                        children[parent.id].forEach(function(child) {
                            var padding = 'padding-left: 20px !important;';
                            var spaces = '&nbsp;&nbsp;'; // Tạo khoảng trắng thay cho dấu gạch ngang
                            var childOption = $('<option>', {
                                value: child.id,
                                text: spaces + child.name,  // Sử dụng khoảng trắng thay cho dấu gạch ngang
                                style: padding // Đảm bảo mục con thụt vào dưới mục cha với !important
                            });
                            ...
                            if (children[child.id]) {
                                children[child.id].forEach(function(grandChild) {
                                    var grandChildOption = $('<option>', {
                                        value: grandChild.id,
                                        text: spaces + spaces + grandChild.name,  // Thêm khoảng trắng 2 lần để thụt vào thêm một cấp
