...
$(document).ready(function() {
    ...
    $(document).ready(function() {
        $('#createAttribute').on('click', function(event) {
            ...
            // Mã khác để xử lý popup
            $('#attributeModal').modal('show');  // Hiển thị modal khi bấm vào nút
        });
    });
...
// Hiển thị danh sách tìm kiếm khi click vào input
categoryInput.addEventListener('click', () => {
    searchInput.value = '';  // Làm mới thanh tìm kiếm khi mở
    dropdownList.classList.add('show-search');
    searchInput.focus();
});

// Ẩn dropdown nếu người dùng click ra ngoài
document.addEventListener('click', (event) => {
    if (!categoryInput.contains(event.target) && !dropdownList.contains(event.target)) {
        dropdownList.classList.remove('show-search');
    }
});

// Lọc kết quả khi người dùng nhập vào thanh tìm kiếm
...
dropdownItems.forEach(item => {
    item.addEventListener('click', () => {
        categoryInput.value = item.textContent;  // Hiển thị nội dung đã chọn
        dropdownList.classList.remove('show-search');
    });
});
