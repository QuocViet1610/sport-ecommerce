...
addAttributeBtn.addEventListener("click", () => {
    ...
    select.classList.add("attribute-dropdown", "js-example-basic-single");  // Thêm class cho select
    
    // Thêm select vào DOM (có thể thay đổi phần tử này nếu cần)
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.code == 200) {
                ...
                const defaultOption = $("<option>").val("").text("Chọn thuộc tính").prop("selected", true);
                select.append(defaultOption);
    
                // Duyệt qua dữ liệu trả về từ API và thêm các tùy chọn vào select
                attributes.forEach(attribute => {
                    const option = $("<option>").val(attribute.id).text(attribute.name);
                    select.append(option);
                });
    
                // Khởi tạo Select2 cho select sau khi đã thêm tất cả các tùy chọn
                $(select).select2();
    
                // Lắng nghe sự kiện thay đổi trên select2
                $(select).on("change", function() {
                    const selectedAttributeId = $(this).val(); // Lấy ID thuộc tính đã chọn
                    console.log('ID thuộc tính đã chọn:', selectedAttributeId);
    
                    // Hiển thị ID trong một div hoặc làm gì đó với ID
                    document.getElementById('selectedIdDisplay').textContent = `ID được chọn: ${selectedAttributeId}`;
                });
